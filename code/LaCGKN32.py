'''
LaCGKN Forecast + DA + UQ. (dimz=32*32*2)
'''
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = CircularConv2d(in_channels, out_channels, 3, 1)
        self.act = nn.SiLU()
...
class DownBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.down = nn.Sequential(
            CircularConv2d(in_channels, out_channels, 4, 2), # downsample via stride=2
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, mode="bilinear"):
        ...
        self.up = nn.Sequential(
            CircularConvTranspose2d(in_channels, out_channels, 4, 2),
            nn.GroupNorm(groups, out_channels),
            nn.SiLU(),
            )
...
class CGN(nn.Module):
    ...
    def __init__(self, dim_u1, dim_z, use_pos_encoding=True, num_frequencies=6, rank_g2=64):
        ...
        self.f2_param = nn.Parameter(1/dim_z**0.5 * torch.rand(dim_z, 1))  # (dim_z, 1)
        # SVD-like low-rank g2
    ...
    def forward(self, x):                           # x: (B, I, 2)
        ...
        # G2: Project to Stiefel (orthonormal columns)
        U, _ = torch.linalg.qr(self.U_raw, mode='reduced')      # (dim_z, r), U^T U = I
        V, _ = torch.linalg.qr(self.V_raw, mode='reduced')      # (dim_z, r), V^T V = I
        # Nonnegative “singular values”
        sigma = nnF.softplus(self.s_raw) + 1e-6                 # (r,)
        g2 = ((U * sigma) @ V.T + torch.diag(self.diag)).expand(B, -1, -1)  # (B, dim_z, dim_z)
        return [f1, g1, f2, g2]
...
autoencoder = AutoEncoder(in_channels=2, out_channels=2, hidden_channels=32, depth=1).to(device)
...
STAGE1_CKPT_PATH = r"../model/LaCGKN32_stage1_test.pt"
...
np.savez(r"../model/LaCGKN32_stage1_loss_history.npz", **loss_history_stage1)
# """
...
np.save(r"../data/LaCGKN32_xy_unit_OneStepPrediction_stage1.npy", test_u1_pred.to("cpu"))
np.save(r"../data/LaCGKN32_psi_OneStepPrediction_stage1.npy", test_u2_pred.to("cpu"))
...
with torch.no_grad():
    for n in range(1, steps+1):
        ...
        u1_initial = u1_pred
        u2_initial = u2_pred
np.save(r"../data/LaCGKN32_xy_unit_MultiStepPrediction_stage1.npy", test_u1_pred.to("cpu"))
np.save(r"../data/LaCGKN32_psi_MultiStepPrediction_stage1.npy", test_u2_pred.to("cpu"))
...
torch.save(sigma_hat, "../data/LaCGKN32_sigma_hat.pt")
# sigma_hat = torch.load("../data/LaCGKN32_sigma_hat.pt", weights_only=True)
...
def CGFilter_batch(cgkn, sigma, u1, mu0, R0, jitter: float = 1e-6): # u1: (B, Nt, I, 4, 1), mu0: (B, dim_z, 1), R0: (B, dim_z, dim_z)
    device = u1.device
    dtype  = u1.dtype
    B, Nt  = u1.shape[:2]
    ...
    mu_pred[:, 0] = mu0
    for n in range(1, Nt):
        x, y = unit2xy(u1[:, n - 1].squeeze(-1))    # (B, I), (B, I)
        pos  = torch.stack([x, y], dim=-1)          # (B, I, 2)
        ...
        I = torch.linalg.cholesky(S)
        X = torch.cholesky_solve(Cross, I)          # (B, dim_u1, dim_z)
...
STAGE2_CKPT_PATH = r"../model/LaCGKN32_stage2.pt"
...
np.savez(r"../model/LaCGKN32_stage2_loss_history.npz", **loss_history_stage2)
# """
...
torch.save(train_mu_pred, "../data/LaCGKN32_train_mu_pred.pt")
# # Load if train_mu_pred has been saved
# train_mu_pred = torch.load("../data/LaCGKN32_train_mu_pred.pt")
...
# checkpoint = torch.load("../model/UQNet32.pt", map_location=device)
...
test_u1 = test_u1.to("cpu")
test_u2 = test_u2.to("cpu")
np.save(r"../data/LaCGKN32_xy_unit_OneStepPrediction_stage2.npy", test_u1_pred.to("cpu"))
np.save(r"../data/LaCGKN32_psi_OneStepPrediction_stage2.npy", test_u2_pred.to("cpu"))
...
test_u1 = test_u1.to("cpu")
test_u2 = test_u2.to("cpu")
np.save(r"../data/LaCGKN32_psi_DA.npy", test_mu_pred.to("cpu"))
...
np.save(r"../data/LaCGKN32_psi_DA_se.npy", test_mu_se_pred)
